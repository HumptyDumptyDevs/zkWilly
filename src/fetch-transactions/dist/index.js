...
// index.ts
var fetch_transactions_exports = {};
__export(fetch_transactions_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(fetch_transactions_exports);
...
var handler = async (event, context) => {
  try {
    ...
    const blockHash = eventBody.result;
    if (!blockHash) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Block hash not found in event body" })
      };
    }
    const response = await axios_default.post(process.env.RPC_URL, {
      jsonrpc: "2.0",
      method: "eth_getBlockByHash",
      params: [blockHash, true],
      // Include transactions by setting second param to true
      id: 1
    });
